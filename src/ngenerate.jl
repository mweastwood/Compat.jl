...
macro ngenerate(itersym, returntypeexpr, funcexpr)
    ...
    Base.Cartesian.isfuncexpr(funcexpr) || error("Requires a function expression")
...
function _ngenerate(itersym::Symbol, funcexpr::Expr)
    ...
    varname, T = Base.Cartesian.get_splatinfo(prototype, itersym)
    ex = Expr(:$, itersym)
    Base.Cartesian.sreplace!(body, itersym, ex)
    if !isempty(varname)
...
macro nsplat(itersym, args...)
    ...
    Base.Cartesian.isfuncexpr(funcexpr) || error("Second argument must be a function expression")
    prototype = funcexpr.args[1]
    body = funcexpr.args[2]
    varname, T = Base.Cartesian.get_splatinfo(prototype, itersym)
...
function _nsplat(prototype, body, varname, T, itersym)
    ...
    newbody = quote
        ...
        resolvesplats!($bodyquot, $varquot, $itersym)
    end
    prototype, newbody
end
